...
class OptimizedDatabase(Database):
    ...
    async def get_tasks_to_run(
        ...
        try:
            async with await conn.execute(
                ...
                if not route:
                    log.warning(f"Route {route_name} not found")
                    return []
            ...
            for i, module in enumerate(unique_modules):
                ...
                if not task_data:
                    should_run = True
                    log.debug(
                        f"Task {module} for {route_name} never executed before, will run"
                    )
                    ...
                    if error_count < max_attempts:
                        should_run = True
                        log.debug(
                            f"Task {module} has status {task_data['status']}, "
                    ...
                    if task_data["last_executed"] is None:
                        ...
                        if (now - last_executed) >= min_interval:
                            ...
                            if total_seconds > 0:
                                ...
                                log.warning(
                                    f"{module_name} for account {route_name} will be available in "
        ...
        try:
            ...
            async with await conn.execute(
                ...
                if cursor.rowcount == 0:
                    log.warning(
                        f"Task with ID {task_id} does not exist. Skipping update."
                    )
                    return
            ...
            if connection_owned:
                try:
                    ...
                    pass
                log.error(f"Failed to update task {task_id}: {str(e)}")
        ...
        async with cls._db_write_semaphore:
            async with cls.transaction() as conn:
                try:
                    ...
                    log.error(f"Database operation failed during route creation: {str(e)}")
    ...
    async def get_route_stats(cls) -> list[RouteStats]:
        ...
        try:
            ...
            log.error(f"Failed to get route statistics: {str(e)}")
    ...
    async def get_accounts_statistics(
        ...
        try:
            ...
            log.error(f"Failed to retrieve account statistics: {str(e)}")
        ...
        async with cls._db_write_semaphore:
            async with cls.transaction() as conn:
                try:
                    ...
                    log.error(f"Error updating account statistics {address}: {str(e)}")
                    raise
